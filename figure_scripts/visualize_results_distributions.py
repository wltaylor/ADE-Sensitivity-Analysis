...
for i,ax in enumerate(axes):
    data = pd.read_csv(f'/Users/williamtaylor/Documents/Github/ADE-Sensitivity-Analysis/results/metrics_{keys[i]}.csv', index_col=0)
...
for i,key in enumerate(keys):
    ...
    data = pd.read_csv(f'/Users/williamtaylor/Documents/Github/ADE-Sensitivity-Analysis/results/metrics_{key}.csv', index_col=0)
...
for i, key in enumerate(keys):
    ...
    sns.ecdfplot(data=data['Peak'], color=colors[i], label=titles[i])

axes.set_ylabel('% of simulations')
axes.set_xlabel('Time to reach peak concentration (log)')
axes.set_xscale('log')    
axes.legend()
plt.show()


#%% scatter plot of parameters vs time to peak concentration
