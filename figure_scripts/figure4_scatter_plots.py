...
loess = lowess(metrics[:,time_idx], bulk, frac=0.3, return_sorted=True)
axes[2].plot(loess[:, 0], loess[:, 1], color='black', linewidth=2)
#axes[2].set_xscale('log')
axes[2].set_title('(c)', fontweight='bold', fontsize=16, loc='left')
...
loess_alpha = lowess(metrics[:,time_idx], alpha, frac=0.3, return_sorted=True)
axes[1].plot(loess_alpha[:, 0], loess_alpha[:, 1], color='black', linewidth=2)
axes[1].set_xscale('log')
axes[1].set_title('(b)', fontweight='bold', fontsize=16, loc='left')
axes[1].set_xlabel(r'$\alpha$', fontsize=16)

axes[2].scatter(dispersivity, metrics[:,time_idx])
loess = lowess(metrics[:,time_idx], dispersivity, frac=0.3, return_sorted=True)
axes[2].plot(loess[:, 0], loess[:, 1], color='black', linewidth=2)
axes[2].set_xscale('log')
axes[2].set_title('(c)', fontweight='bold', fontsize=16, loc='left')
...
loess = lowess(metrics[:,time_idx], decay, frac=0.3, return_sorted=True)
axes[3].plot(loess[:, 0], loess[:, 1], color='black', linewidth=2)
...
loess = lowess(metrics[:,time_idx], bulk, frac=0.3, return_sorted=True)
axes[4].plot(loess[:, 0], loess[:, 1], color='black', linewidth=2)
